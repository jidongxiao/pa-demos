...
class RoomManager {
    ...
    try {
        ...
        // Step 2: Listen for join outcome
        const onRoomJoined = (data) => {
            this.wsClient.off('room_full', onRoomFull);
        ...
        const onRoomFull = (data) => {
            this.wsClient.off('room_joined', onRoomJoined);
            this.showJoinError(data.message || 'Room is full. Please try again later.');
        };

        this.wsClient.on('room_joined', onRoomJoined);
        this.wsClient.on('room_full', onRoomFull);

        // Step 3: Send actual join request
        this.wsClient.send('join_room', { room_code: roomCode });
